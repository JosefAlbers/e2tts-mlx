...
def get_batch_rep(dataset, batch_size, dataset_to_gen):
    rep_size = int(batch_size * 0.75)
    gen_size = batch_size - rep_size
    rep_iter = iter(dataset.shuffle(seed=42))
    gen_iter = iter(dataset_to_gen.shuffle(seed=42))
    while True:
        batch_mel = []
        batch_txt = []
...
def main(prompt=False, batch_size=30, n_epoch=100, lr=2e-4, dim=512, n_head=8, depth=8, n_ode=1, dim_dur=32, n_head_dur=1, dep_dur=2, mxl=1025, path_ds='JosefAlbers/cmu-arctic', split='aew', postfix=''):
    if prompt:
        tts(prompt)
        return prompt
